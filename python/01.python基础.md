...
In [16]: fun(1,2,*args,**kwargs) # 注意* 和** 的使用
a= 1
b= 2
c= (3, 4, 5)
d=
...
有该属性的模块，在被其他模块引用时（使用from 文件夹名称 import * 导入时，该导入不用填写模块名可以直接调用方法），会忽略该模块中指定名称的元素，如方法，函数，变量和类，如果在其他模块中调用，那么会报错
...
```py
__all__ = ["test1","test2"] # 这里的all的含义与上面的有所不同，有点相反
```
